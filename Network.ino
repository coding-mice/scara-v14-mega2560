...
void ESPsendCommand(String command, String stopstr, int timeout_secs)
{
...
void MsgRead()
{
  ...
  while ((Serial1.available() > 0) && ((micros() - t0) < 5000)) {
    // We rotate the Buffer (we could implement a ring buffer in the future)
...
void ParseMsg(uint8_t interface)
{
  ...
  if ((char(MsgBuffer[0]) == 'J') && (char(MsgBuffer[1]) == 'J') && (char(MsgBuffer[2]) == 'A') && (char(MsgBuffer[3]) == 'H')) {
    ...
    newMessage = 0; // No message to process in the main code...
    working = false;
  ...
  if ((char(MsgBuffer[0]) == 'J') && (char(MsgBuffer[1]) == 'J') && (char(MsgBuffer[2]) == 'A') && (char(MsgBuffer[3]) == 'T')) {
    ...
    trajectory_processing = true;   // trajectory processing mode...
    working = false;  
